...
import { addFavoriteCity, removeFavoriteCity } from "./../Redux/actions/index";
import {darkMode, setDarkMode} from '../Pages/Homepage'

function SideOne({darkMode, setDarkMode}) {
  ...
  const currentCityName = useSelector((state) => state.weatherReducer && state.weatherReducer.cityName);
  ...
  const toggleTempUnit = () => {
    setTempUnit(prevTempUnit => prevTempUnit === 'C' ? 'F' : 'C');
  }
  useEffect(() => {
    setIsFavoriteCity( currentWeatherData && favoriteCities.includes(currentWeatherData.name));
  }, [currentWeatherData, favoriteCities]);


  
  const handleFavoriteClick = () => {
    if (isFavoriteCity) {
      dispatch(removeFavoriteCity(currentWeatherData.name));
    } else {
      dispatch(addFavoriteCity(currentWeatherData.name));
    }
  };
  ...
  return (
    <section className={`h-full md:h-screen w-screen md:w-2/5 xl:w-3/12 ${darkMode ? "bg-black text-white" : "bg-base-200 text-white"}`}>
      {/* alert box */}
      {
      ...
      <div className="card bg-base-content text-neutral-content w-11/12 mx-auto mt-5 mb-5">
        <div className="card-body items-center text-center">
          <div className="font-sans text-5xl text-black">
            {data.name}
            <button onClick={handleFavoriteClick}>
              <i className={`fa fa-star ${isFavoriteCity ? 'text-yellow-500' : ''}`} />
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="text-center mb-5">
        <img
          ...
          className="mx-auto"
        />
        <div className="font-sans text-6xl font-bold mb-5">
          {
            tempUnit === 'C' ? data.temp : (data.temp * 9 / 5) + 32
          }° {tempUnit}
        </div>
        <button onClick={toggleTempUnit} className="btn">
